...
function add_token(p, token) {
    if (
        (p.tokens[p.len] === LIST_ORDERED ||
            p.tokens[p.len] === LIST_UNORDERED) &&
        token !== LIST_ITEM
    ) {
        end_token(p);
    }
                    ...
                    case "#":
                        switch (char) {
                            ...
                            case " ":
                                ...
                                add_token(
                                    p,
                                    heading_from_level(p.pending.length),
                                );
                                clear_root_pending(p);
                                continue;
                        }
            ...
            case CODE_INLINE:
                switch (char) {
                    case "`":
                        if (
                            pending_with_char.length ===
                            p.fence_start + Number(p.pending[0] === " ")
                        ) {
                            add_text(p);
                            end_token(p);
                            p.pending = "";
...
`
            ...
            case MAYBE_URL:
                if (
                    pending_with_char === "http://" ||
                    pending_with_char === "https://"
                ) {
                    add_text(p);
                    add_token(p, RAW_URL);
                    p.pending = pending_with_char;
...
function default_renderer(root) {
    return {
        ...
        data: {
            nodes: [root, undefined, undefined, undefined, undefined],
            index: 0,
        },
    };
}
...
class StreamingMarkdownElement extends HTMLElement {
    ...
    finish() {
        if (this.parser) parser_end(this.parser);
    ...
    autoScrollIfNearBottom() {
        const nearBottom =
            this.scrollHeight - this.scrollTop - this.clientHeight < 64;
        if (nearBottom) this.scrollTop = this.scrollHeight;
    }
}
